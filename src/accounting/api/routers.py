...
from src.accounting.infrastructure.database.session import get_session
...
async def create_account(amount: float, currency: str, db=get_session()):
...
async def commit_transaction(accountId: str, amount: float, currency: str, description: str, category: str, transactionType: str, db=get_session()):
